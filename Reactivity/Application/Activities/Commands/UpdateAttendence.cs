...
namespace Application.Activities.Commands
{
    public class UpdateAttendence
    {
        ...
        public class Handler(IUserAccessor userAccesor, AppDbContext context) : IRequestHandler<Command, Result<Unit>>
        {
            public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (activity == null) return Result<Unit>.Failure("Wydarzenie ze znalezione", 404);
                ...
                return result
                    ? Result<Unit>.Success(Unit.Value) : Result<Unit>.Failure("Wystąpił problem podczas zapytania", 400);
            }
        }
    }
}
